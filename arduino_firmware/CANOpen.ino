...
void CANOpen::begin() {
  ...
  CANopen.sendHeartbeatMessage(CANbus.getAddress(), operationalState);
...
void CANOpen::handleNMTMessage(CANOpen_msg_t *msg) {
    ...
    if(nodeID != CANbus.getAddress()) return;
...
void CANOpen::updateHeartbeat() {
  ...
  if (currentTime - lastHeartbeatTime >= HEARTBEAT_INTERVAL_MS) {
      this->sendHeartbeatMessage(CANbus.getAddress(), operationalState);
